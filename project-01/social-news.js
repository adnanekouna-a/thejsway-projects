...
while(1) {
	...
	if(Number(actionChoice) == 1) {
		...
		let removedLink = 0;
		do {
			removedLink = prompt(removeLinkMsg+between(1,links.length||1));
		} while(isNumberUnvalid(removedLink, 0, links.length+1));
...
// - Add the functionality to add https:// in links if not there
